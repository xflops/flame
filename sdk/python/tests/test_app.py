...
def test_runpy_resolves_object_ref_to_cached_none():
    ...
    with patch("flamepy.app.runpy.get_object", return_value=None):
...
def test_runpy_binds_session_context_and_publishes_invocation_attributes():
    import flamepy.app as app
    from flamepy.app.runpy import FlameRunpyService
    ...
    svc = FlameRunpyService()
    svc._load_app_context = lambda: ServiceContext(Worker, constructor_args=())
    ...
    with patch("flamepy.app.runpy.put_object", return_value=object_ref) as put:
...
def test_runpy_keeps_concurrent_invocation_attributes_with_their_response():
    import threading

    import flamepy.app as app
    from flamepy.app.runpy import FlameRunpyService
    from flamepy.core.service import ApplicationContext, FlameInstanceServicer, SessionContext
    ...
    # Ensure both RPCs finish their service invocation before either response
    # consumes attributes. A process-wide publisher would combine the two sets.
    response_barrier = threading.Barrier(2, timeout=5)
    take_attributes = svc._take_attributes

    def synchronized_take_attributes():
        response_barrier.wait()
        return take_attributes()

    svc._take_attributes = synchronized_take_attributes
    responses = [None, None]
    ...
    def invoke(index):
        ...
        responses[index] = servicer.OnTaskInvoke(request, MagicMock())
    ...
    with patch("flamepy.app.runpy.put_object", return_value=result_ref):
...
def test_runpy_binds_recursive_service_to_current_session(monkeypatch):
    import flamepy.app as app
    from flamepy import FlameError
    from flamepy.app.runpy import FlameRunpyService
    from flamepy.core.service import ApplicationContext, SessionContext, TaskContext
    ...
    svc._load_app_context = lambda: ServiceContext(Worker, constructor_args=())
    session_context = SessionContext(None, "recursive-session", ApplicationContext("recursive-app"))
    svc.on_session_enter(session_context)

    request = ServiceRequest(method="run")
    result_ref = SimpleNamespace(encode=lambda: b"result-ref")
    with patch("flamepy.app.runpy.put_object", return_value=result_ref):
        result = svc.on_task_invoke(TaskContext("task", "recursive-session", cloudpickle.dumps(request)))
...
def test_runpy_resets_recursive_session_context_after_failure(monkeypatch):
    import flamepy.app as app
    from flamepy import FlameError
    from flamepy.app.runpy import FlameRunpyService
    from flamepy.core.service import ApplicationContext, SessionContext, TaskContext
    ...
    svc = FlameRunpyService()
    svc._load_app_context = lambda: ServiceContext(Worker, constructor_args=())
    svc.on_session_enter(SessionContext(None, "recursive-session", ApplicationContext("recursive-app")))
    request = ServiceRequest(method="run")

    with pytest.raises(RuntimeError, match="service failed"):
        svc.on_task_invoke(TaskContext("task", "recursive-session", cloudpickle.dumps(request)))
...
def test_app_runtime_helpers_are_invocation_scoped():
    import flamepy.app as app
    from flamepy.app.runpy import FlameRunpyService
    from flamepy.core.service import ApplicationContext, SessionContext, TaskContext
    ...
    result_ref = SimpleNamespace(encode=lambda: b"result-ref")
    with patch("flamepy.app.runpy.put_object", return_value=result_ref):
        result = svc.on_task_invoke(TaskContext("task", "session", cloudpickle.dumps(ServiceRequest(method="run"))))
    assert result == b"result-ref"
    assert set(svc._take_attributes().attr) == {b"initial", b"next"}

    with pytest.raises(TypeError, match="must contain bytes"):
        svc.on_task_invoke(TaskContext("task", "session", cloudpickle.dumps(ServiceRequest(method="invalid"))))
    assert list(svc._take_attributes().attr) == []
...
def test_runpy_uses_constructor_args_to_distinguish_class_from_function():
...
def test_runpy_reuses_class_execution_object_between_sessions():
    ...
    svc._load_app_context = lambda: next(contexts)
    session = SessionContext(None, "session", ApplicationContext("app"))
...
def test_runpy_does_not_reuse_functions():
    from flamepy.app.runpy import FlameRunpyService
...
def test_runpy_distinguishes_constructed_handles_by_service_id():
    from flamepy.app.runpy import FlameRunpyService
