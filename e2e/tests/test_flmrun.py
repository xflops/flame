...
def test_flmrun_no_args():
    ...
    # Create a session with the function
    ctx = runner.RunnerContext(execution_object=get_message_func)
    common_data_bytes = serialize_runner_context(ctx, FLMRUN_E2E_APP)
    ssn = flamepy.create_session(FLMRUN_E2E_APP, common_data_bytes)

    try:
        # Invoke with no arguments (all fields None)
        req = runner.RunnerRequest(method=None)
        req_bytes = serialize_runner_request(req)
        result_bytes = ssn.invoke(req_bytes)
        result = get_object(ObjectRef.decode(result_bytes))
...
def test_flmrun_multiple_tasks():
    ...
    # Create a session with the function
    ctx = runner.RunnerContext(execution_object=multiply_func)
    common_data_bytes = serialize_runner_context(ctx, FLMRUN_E2E_APP)
    ssn = flamepy.create_session(FLMRUN_E2E_APP, common_data_bytes)
    ...
    try:
        ...
        for args, expected in test_cases:
            req = runner.RunnerRequest(method=None, args=args)
            req_bytes = serialize_runner_request(req)
            result_bytes = ssn.invoke(req_bytes)
            result = get_object(ObjectRef.decode(result_bytes))
...
def test_flmrun_lambda_function():
    ...
    ctx = runner.RunnerContext(execution_object=square_func)
    common_data_bytes = serialize_runner_context(ctx, FLMRUN_E2E_APP)
    ssn = flamepy.create_session(FLMRUN_E2E_APP, common_data_bytes)
    ...
    try:
        ...
        for x in [2, 5, 10, 15]:
            req = runner.RunnerRequest(method=None, args=(x,))
            req_bytes = serialize_runner_request(req)
            result_bytes = ssn.invoke(req_bytes)
            result = get_object(ObjectRef.decode(result_bytes))
            expected = x * x
...
def test_flmrun_complex_return_types():
    ...
    # Test dict return
    ctx = runner.RunnerContext(execution_object=return_dict_func)
    common_data_bytes = serialize_runner_context(ctx, FLMRUN_E2E_APP)
    ssn = flamepy.create_session(FLMRUN_E2E_APP, common_data_bytes)
    try:
        req = runner.RunnerRequest(method=None, args=("test", 42))
        req_bytes = serialize_runner_request(req)
        result_bytes = ssn.invoke(req_bytes)
        result = get_object(ObjectRef.decode(result_bytes))
    ...
    # Test list return
    ctx = runner.RunnerContext(execution_object=return_list_func)
    common_data_bytes = serialize_runner_context(ctx, FLMRUN_E2E_APP)
    ssn = flamepy.create_session(FLMRUN_E2E_APP, common_data_bytes)
    try:
        req = runner.RunnerRequest(method=None, args=(5,))
        req_bytes = serialize_runner_request(req)
        result_bytes = ssn.invoke(req_bytes)
        result = get_object(ObjectRef.decode(result_bytes))
    ...
    # Test tuple return
    ctx = runner.RunnerContext(execution_object=return_tuple_func)
    common_data_bytes = serialize_runner_context(ctx, FLMRUN_E2E_APP)
    ssn = flamepy.create_session(FLMRUN_E2E_APP, common_data_bytes)
    try:
        req = runner.RunnerRequest(method=None, args=(123, "test"))
        req_bytes = serialize_runner_request(req)
        result_bytes = ssn.invoke(req_bytes)
        result = get_object(ObjectRef.decode(result_bytes))
...
def test_flmrun_runner_request_validation():
    ...
    req = runner.RunnerRequest(method=None, kwargs={"a": 1})
    assert req.args is None
    assert req.kwargs == {"a": 1}

    # Test that both args and kwargs can be set together
    req = runner.RunnerRequest(method=None, args=(1, 2), kwargs={"a": 1})
    assert req.args == (1, 2)
    assert req.kwargs == {"a": 1}
