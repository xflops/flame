...
These notes cover the changes between `v0.5.0` and the latest `release-0.6` branch snapshot. The earlier `v0.6.0` tag points to `4f86311e`; use `v0.6.0-rc2` and `release-0.6` as the current release-candidate source until the final stable tag is updated.
...
- Rust crates (`stdng`, `flame-rs-macros`, and `flame-rs`) have not been published for this release candidate yet.
