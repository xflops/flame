...
- `flamepy 0.6.0` is not published to PyPI yet; the latest verified 0.6 PyPI artifact remains `flamepy 0.6.0rc2`: https://pypi.org/project/flamepy/0.6.0rc2/.
- Verified PyPI rc2 artifact hashes:
...
- Final Docker image tags are not published yet; the current verified Docker image tags remain `v0.6.0-rc2` for `linux/arm64` only:
...
- Rust crate status: final `flame-rs 0.6.0` and `flame-rs-macros 0.6.0` crates are not published yet; `flame-rs 0.6.0-rc2` remains published on crates.io and uses `stdng 0.1.8` plus optional `flame-rs-macros 0.6.0-rc1`.
- Remaining release gap: publish final PyPI, Rust, and multi-arch Docker artifacts before announcing the stable release.
